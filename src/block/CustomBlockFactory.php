...
use pocketmine\block\permutations\Permutable;
use pocketmine\block\permutations\Permutation;
use pocketmine\block\permutations\Permutations;
...
use pocketmine\scheduler\AsyncRegisterBlocksTask;
		...
		if($block instanceof Permutable) {
			...
			foreach(Permutations::getCartesianProduct($blockPropertyValues) as $meta => $permutations){
				...
				foreach($permutations as $i => $value){
					$states->setTag($blockPropertyNames[$i], CustomNBT::getTagType($value));
				}
