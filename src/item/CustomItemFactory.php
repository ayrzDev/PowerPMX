...
final class CustomItemFactory
{
	...
	public function registerItem(string $className, string $identifier, string $name): void
	{
		...
		if (($componentBased = $item instanceof ItemComponents)) {
			$this->itemComponentEntries[$identifier] = new ItemComponentPacketEntry(
				$identifier,
				new CacheableNbt(
					$item->getComponents()
						->setInt("id", $itemId)
		...
		$parts = explode(":", $identifier); // ":" karakterine göre ayır
		...
		VanillaItems::register($parts, CustomItemFactory::getInstance()->get($identifier));
	}
